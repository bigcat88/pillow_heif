...
then
  wget https://github.com/Kitware/CMake/releases/download/v"$1"/cmake-"$1"-linux-x86_64.sh \
  && chmod +x cmake-"$1"-linux-x86_64.sh \
  && sh cmake-"$1"-linux-x86_64.sh --prefix=/usr/local/ --exclude-subdir
elif [[ $os_architecture == "aarch64" ]]
then
  wget https://github.com/Kitware/CMake/releases/download/v"$1"/cmake-"$1"-linux-aarch64.sh \
