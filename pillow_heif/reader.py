...
def open(
    fp, *, apply_transformations=True, convert_hdr_to_8bit=True
...
def _read_heif_context(ctx, d, apply_transformations, convert_hdr_to_8bit):
    error = _libheif.lib.heif_context_read_from_memory_without_copy(
        ctx, d, len(d), _libheif.ffi.NULL
    )
    if error.code != 0:
        raise _error.HeifError(
            code=error.code,
            subcode=error.subcode,
            message=_libheif.ffi.string(error.message).decode(),
        )
...
def _read_metadata(handle):
    block_count = _libheif.lib.heif_image_handle_get_number_of_metadata_blocks(
        handle, _libheif.ffi.NULL
    )
    if block_count == 0:
        return []
    metadata = []
    ids = _libheif.ffi.new("heif_item_id[]", block_count)
    _libheif.lib.heif_image_handle_get_list_of_metadata_block_IDs(
        handle, _libheif.ffi.NULL, ids, block_count
    )
    for each_item in ids:
        metadata_type = _libheif.lib.heif_image_handle_get_metadata_type(
            handle, each_item
        )
        metadata_type = _libheif.ffi.string(metadata_type).decode()
        data_length = _libheif.lib.heif_image_handle_get_metadata_size(
            handle, each_item
        )
        if data_length > 0:
            p_data = _libheif.ffi.new("char[]", data_length)
            error = _libheif.lib.heif_image_handle_get_metadata(
                handle, each_item, p_data
            )
            if error.code != 0:
                raise _error.HeifError(
                    code=error.code,
...
def _read_color_profile(handle):
    ...
    if profile_type == _constants.heif_color_profile_type_not_present:
        return None
    if profile_type in (
        _constants.heif_color_profile_type_rICC,
        _constants.heif_color_profile_type_prof,
    ):
...
def _read_heif_image(handle, heif_file):
    ...
    error = _libheif.lib.heif_decode_image(
        handle,
        p_img,
        colorspace,
        chroma,
    ...
    p_data = _libheif.lib.heif_image_get_plane_readonly(
