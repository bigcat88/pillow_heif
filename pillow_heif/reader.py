...
def _read_heif_context(ctx, d, apply_transformations, convert_hdr_to_8bit):
    ...
    if error.code != 0:
        raise _error.HeifError(
            code=error.code, subcode=error.subcode, message=_libheif_cffi.ffi.string(error.message).decode(),)
...
def _read_heif_handle(handle, apply_transformations, convert_hdr_to_8bit):
    ...
    error = _libheif_cffi.lib.heif_decode_image(handle, p_img, colorspace, chroma, p_options,)
...
def _read_metadata(handle):
    ...
    _libheif_cffi.lib.heif_image_handle_get_list_of_metadata_block_IDs(handle, _libheif_cffi.ffi.NULL, ids, block_count)
...
def _read_color_profile(handle):
    ...
    if profile_type == _constants.heif_color_profile_type_rICC:
        ...
        error = _libheif_cffi.lib.heif_image_handle_get_nclx_color_profile(handle, pp_data)
        p_data = pp_data[0]
        _libheif_cffi.ffi.release(pp_data)
    else:
        raise _error.HeifError(
