...
def test_exif_overwriting():
    ...
    saved_image = Image.open(out_buf)
    for i, frame in enumerate(ImageSequence.Iterator(saved_image)):
        assert frame.getexif() if i else not frame.getexif()
    exif_data = saved_image.getexif()
    ...
    out_buf2 = BytesIO()
    image.save(out_buf2, format="HEIF", exif=exif_data.tobytes(), save_all=True)  # change Exif in primary image
    saved_image = Image.open(out_buf2)
    for i, frame in enumerate(ImageSequence.Iterator(saved_image)):
        exif_data = frame.getexif()
        if not i:
