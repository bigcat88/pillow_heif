...
@pytest.mark.parametrize("mode", ("L", "RGB", "RGBA"))
def test_to_pillow_borrows_only_a_matching_plane(mode):
    data = encoded(mode, (128, 64))
    # `RGB` is three bytes per pixel and Pillow keeps four, so only the Pillow layout can be borrowed
    assert borrowed(pillow_heif.open_heif(BytesIO(data))[0].to_pillow()) is (mode != "RGB")
