...
#define LIBHEIF_NUMERIC_VERSION ((1<<24) | (19<<16) | (3<<8) | 0)
...
struct heif_writer
{
  ...
  // --- version 1 functions ---
  struct heif_error (* write)(struct heif_context* ctx, // TO-DO: why do we need this parameter?
                              const void* data,
                              size_t size,
                              void* userdata);
};
...
LIBHEIF_API
int heif_context_get_encoder_descriptors(struct heif_context*, // TO-DO: why do we need this parameter?
...
// You will have to query that further (TO-DO) or just try to decode and check the returned error.
LIBHEIF_API
